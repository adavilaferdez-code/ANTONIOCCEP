...
// Bonification Rules (Purchased -> Gift)
const bonificationRules = {
    71: { buy: 2, gift: 5 },
    62: { buy: 5, gift: 3 },
    50: { buy: 1, gift: 1 }
};
...
function addToGiftCart(id, name, price) {
    ...
    if (existing) {
        ...
        calcCartGift.push({
            ...
            price: price,
            qty: 1,
            discount: initialDiscounts[safeId] || 0 // Use predefined discount
        });
    }
...
function calculateBonification() {
    ...
    // Average price PER BOX (Total Invoice / Total Qty)
    const avgPrice = totalQty > 0 ? totalPaid / totalQty : 0;

    // Update UI
    const bonResultPrice = document.getElementById('bonResultPrice');
    const bonPaidQty = document.getElementById('bonPaidQty');
    const bonFreeQty = document.getElementById('bonFreeQty');
    const bonResultTotal = document.getElementById('bonResultTotal');
    const calcTotalGift = document.getElementById('calcTotalGift');

    if (bonResultPrice) bonResultPrice.textContent = avgPrice.toFixed(2) + ' €';
    if (bonPaidQty) bonPaidQty.textContent = totalQtyPaid;
...
window.checkParamsBonification = checkParamsBonification;
...
async function fetchNearbyPlaces(lat, lon, radiusKm, onlyOpen = false) {
    ...
    // Overpass query for bars, restaurants, cafes, fast food, pubs
    const query = `
        [out:json][timeout:25];
        (
            node["amenity"~"bar|restaurant|cafe|pub|fast_food|ice_cream|nightclub|biergarten"](around:${radiusM},${lat},${lon});
            way["amenity"~"bar|restaurant|cafe|pub|fast_food|ice_cream|nightclub|biergarten"](around:${radiusM},${lat},${lon});
            relation["amenity"~"bar|restaurant|cafe|pub|fast_food|ice_cream|nightclub|biergarten"](around:${radiusM},${lat},${lon});
        );
        out center;
    `;
